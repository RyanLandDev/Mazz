...
module.exports = class extends Command {
  ...
  async run(msg) {
    ...
    const description = [];
    if (this.client.finalizers.get('commandCooldown').getCooldown(msg, this.client.commands.get('work')).remainingTime) description.push(`**Work** | ${workCDh === 0 ? '' : `${workCDh}h`}${workCDm === 0 ? '' : `${workCDm}m`}${workCDs}s`); else description.push('**Work** | **Ready**');
    if (this.client.finalizers.get('commandCooldown').getCooldown(msg, this.client.commands.get('crime')).remainingTime) description.push(`**Crime** | ${crimeCDh === 0 ? '' : `${crimeCDh}h`}${crimeCDm === 0 ? '' : `${crimeCDm}m`}${crimeCDs}s`); else description.push('**Crime** | **Ready**');
    if (this.client.finalizers.get('commandCooldown').getCooldown(msg, this.client.commands.get('beg')).remainingTime) description.push(`**Beg** | ${begCDh === 0 ? '' : `${begCDh}h`}${begCDm === 0 ? '' : `${begCDm}m`}${begCDs}s`); else description.push('**Beg** | **Ready**');
    if (this.client.finalizers.get('commandCooldown').getCooldown(msg, this.client.commands.get('search')).remainingTime) description.push(`**Search** | ${searchCDh === 0 ? '' : `${searchCDh}h`}${searchCDm === 0 ? '' : `${searchCDm}m`}${searchCDs}s`); else description.push('**Search** | **Ready**');
    if (this.client.finalizers.get('commandCooldown').getCooldown(msg, this.client.commands.get('rob')).remainingTime) description.push(`**Rob** | ${robCDh === 0 ? '' : `${robCDh}h`}${robCDm === 0 ? '' : `${robCDm}m`}${robCDs}s`); else description.push('**Rob** | **Ready**');
