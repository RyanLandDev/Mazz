...
module.exports = class extends Command {
  ...
  show(message, [key]) {
    const path = this.client.gateways.users.getPath(key, { onlyConfigurable: true, errors: false, piece: null });
  ...
  async set(message, [key, ...valueToSet]) {
    const status = await message.author.settings.update(key, valueToSet.join(' '), message.guild, { onlyConfigurable: true, arrayAction: 'add' });
  ...
  async remove(message, [key, ...valueToRemove]) {
    const status = await message.author.settings.update(key, valueToRemove.join(' '), message.guild, { onlyConfigurable: true, arrayAction: 'remove' });
    return this.check(message, key, status) || message.sendLocale('COMMAND_CONF_UPDATED', [key, message.author.settings.resolveString(message, status.updated[0].piece)]);
  }
